...
if "**501**" not in zeilen[-1]: fehler.append(f"Summenzeile Aufrufe != 501: {zeilen[-1]}")
if auf!=501: fehler.append(f"Aufrufe addieren zu {auf}, nicht 501")
if not 500<=usd<=520: fehler.append(f"USD addieren zu {usd}, ausserhalb 'rund 510'")
...
m2=re.search(r"ergeben 501 Aufrufe damit \*\*rund (\d+) Stunden\*\*",K)
if not m2: fehler.append("Konzept nennt keine Gesamtstundenzahl fuer 501 Aufrufe")
...
phasen={"R0":23,"R1":110,"R1b":100,"R2":40,"R3":110,"R4":103,"R5":9,"R6":6}
